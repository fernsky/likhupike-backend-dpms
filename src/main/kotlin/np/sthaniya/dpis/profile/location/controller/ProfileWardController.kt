...
@Tag(name = "Ward Management", description = "Endpoints for managing wards within the municipality")
@RestController
@RequestMapping("/api/v1/profile/location/wards")
class ProfileWardController(
        private val wardService: ProfileWardService,
        ...
        @ApiResponses(
                value =
                        [
                                ...
                                SwaggerResponse(
                                        responseCode = "409",
                                        description = "Ward number already exists"
                                )]
        )
        @PostMapping
                ...
                return ResponseEntity.status(HttpStatus.CREATED)
                        .body(
                                ApiResponse.success(
                                        data = ward,
                                        message =
                                                i18nMessageService.getMessage("ward.create.success")
                                )
                        )
        }
        ...
        @GetMapping
        // No authorization required for GET requests
