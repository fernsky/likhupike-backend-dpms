...
/**
 * Configuration class for environment variable loading and verification.
 * Loads environment variables from .env file if it exists and system 
    ...
    fun init() {
        ...
        if (envFile.exists()) {
            try {
                ...
                logger.info("Database URL: ${maskSensitiveInfo(System.getProperty("spring.datasource.url") ?: System.getenv("SPRING_DATASOURCE_URL") ?: "not set")}")
                logger.info("Redis host: ${System.getProperty("redis.host") ?: System.getenv("REDIS_HOST") ?: "not set"}")
                
            } catch (e: Exception) {
                logger.error("Failed to load environment variables from .env file: ${e.message}")
            }
        } else {
            logger.warn(".env file not found in ${System.getProperty("user.dir")}. Using system environment variables.")
        }
    }
